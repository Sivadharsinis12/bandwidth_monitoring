...
# Settings API Routes for Data Limits
@app.get("/api/settings")
async def get_settings():
    from database import get_all_settings, get_today_usage, get_monthly_usage
    settings = get_all_settings()
    from traffic_monitor import get_data_limits, get_current_usage
    
    limits = get_data_limits()
    usage = get_current_usage()
    
    return {
        "daily_limit_gb": limits['daily_limit_gb'],
        "monthly_limit_gb": limits['monthly_limit_gb'],
        "daily_usage_gb": usage['daily_gb'],
        "monthly_usage_gb": usage['monthly_gb'],
        "is_blocked": device_stats.get("is_blocked", False)
    }

@app.post("/api/settings")
async def save_settings(daily_limit_gb: float = 0, monthly_limit_gb: float = 0):
    from database import save_setting
    save_setting('daily_limit_gb', str(daily_limit_gb))
    save_setting('monthly_limit_gb', str(monthly_limit_gb))
    return {"status": "success", "daily_limit_gb": daily_limit_gb, "monthly_limit_gb": monthly_limit_gb}

@app.post("/api/reset-usage")
async def reset_usage():
    from database import reset_usage
    reset_usage()
    return {"status": "success", "message": "Usage data has been reset"}
